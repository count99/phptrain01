...
$del = "/images/firm/".$row_Recordset1['f_pic'];
mysql_free_result($Recordset1);

if ((isset($_GET['f_seq'])) && ($_GET['f_seq'] != "")) {
	
  $deleteSQL = sprintf("DELETE FROM firm WHERE f_seq=%s",
                       GetSQLValueString($_GET['f_seq'], "int"));

  mysql_select_db($database_shopping, $shopping);
  $Result1 = mysql_query($deleteSQL, $shopping) or die(mysql_error());
  if(file_exists($del)){
	  unlink($del);}
	header("Location:f_admin.php?good=1");	  
}
